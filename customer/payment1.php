...
$bid=$row['bid'];
$result1=mysqli_query($con,"select total as tot,ph_id as pid,quantity as Qty from tbl_booking2 where b_id=$bid");
$row1=mysqli_fetch_array($result1);
$total=$row1['tot'];
$pid=$row1['pid'];
$Qty=$row1['Qty'];
$result2=mysqli_query($con,"select * from tbl_photograph where ph_id=$pid");
...
$pdate=date("Y/m/d");
$tot=$amount+50;
//echo "insert into tbl_payment1(bill_no,c_id,date,amount)value('$billno',$cust_id,'$pdate',$tot)";
mysqli_query($con,"insert into tbl_payment1(biil_no,c_id,date,b_id,Amount)value('$billno',$cust_id,'$pdate',$bid,$tot)");
mysqli_query($con,"update tbl_booking2 set status=2 where b_id=$bid");
$quantity=$count-$Qty;
mysqli_query($con,"update tbl_photograph set count=$quantity where ph_id=$pid");
